...
- Le front (hébergé sur `http://localhost:5173`) essaie de faire un fetch(`http://localhost:8080/api/hello`), donc vers un autre port, c’est considéré comme un autre domaine.

- Par défaut, le navigateur bloque la requête s’il ne reçoit pas les en-têtes adéquats du serveur (en particulier Access-Control-Allow-Origin).
